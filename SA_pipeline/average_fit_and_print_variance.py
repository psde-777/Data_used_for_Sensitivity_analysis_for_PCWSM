...
for keyword in keywords:
    ...
    my_file = sys.argv[1] + "saccharification/saccharification_high_1" + ".txt";
    ...
    if os.path.isfile(my_file):
        ...
        for i in range(2,N_files+1):
            my_file = sys.argv[1] + "saccharification/saccharification_high_" + str(i) + ".txt";
            if os.path.isfile(my_file):
                countFound += 1
                count = 0;
                data = np.loadtxt(my_file);
    ...
    #    outFile[:,0] = x[:];
        for i in range(N_files):
            my_file = sys.argv[1] + "saccharification/saccharification_high_" + str(i+1) + ".txt";
            if os.path.isfile(my_file):
                data = np.loadtxt(my_file);
                outFile[:,0] += x[:];
...
var_xyl = 0.
for keyword in keywords:
    print("var = " + str(var_glc_dict[keyword]) + " for " + str(keyword))
    print(keyword)
    ...
    with open(sys.argv[1] + "var_" + keyword + ".txt","w") as f:
        f.write(str(var_glc_dict[keyword]))
    print("var = " + str(var_glc_dict[keyword]))

#with open(sys.argv[1] + "var.txt","w") as f:
#   f.write(str(var_glc_dict[high]))
    exit()
