...
//    while( current_run < par.Nruns+1){//This runs as often as
        ...
        // Clear CBH attack position file contents: Jasmin
        std::ofstream Free_ends_file;
        Free_ends_file.open("Output/enzyme_activity/Free_CBH_ends.txt", std::ofstream::out | std::ofstream::trunc);
        Free_ends_file.close();
...
// Check for correct crystallinity value
    ...
    //    			count_hemi++;
        ...
        if(par.verbose == true){
    	    cout << "Hemi crystalline percentage = " << double(count_crystalline)/double(count_hemi) << "; the parameter given in the file reads: " << par.pct_crystalline_hemi << endl;
        }
...
//                            exit(1);
        ...
        if(par.verbose == true){
        	if(substrate ==1){
    	    	if(cellu[poly_selected].len_poly == 1){
                    ...
                    if (current_run == 1){
                        ofstream Free_ends_file;
                        Free_ends_file.open("Output/enzyme_activity/Free_CBH_ends.txt", std::ios_base::app);
                        Free_ends_file << par.real_time << "\t" << par.free_poly_ends.size() << endl; // Jasmin Prints free ends for CBH attack points
                        Free_ends_file.close();    
                    }
        ...
        if(current_run != 0){
            ...
            //par.output_file = "Output/enzyme_activity/Free_CBH_ends_" + to_string(current_run) + ".txt";       
